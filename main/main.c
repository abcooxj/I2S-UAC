...
static esp_err_t uac_output_cb(uint8_t *buf, size_t len, void *ctx) {
    size_t written = 0;
    i2s_channel_write(tx_handle, buf, len, &written, pdMS_TO_TICKS(20));
    return ESP_OK;
}
...
void app_main(void) {
    {
        i2s_chan_config_t cfg = I2S_CHANNEL_DEFAULT_CONFIG(I2S_NUM_0, I2S_ROLE_SLAVE);
        i2s_new_channel(&cfg, &tx_handle, NULL);
    }
    {
        i2s_chan_config_t cfg = I2S_CHANNEL_DEFAULT_CONFIG(I2S_NUM_1, I2S_ROLE_SLAVE);
        i2s_new_channel(&cfg, NULL, &rx_handle);
    }
